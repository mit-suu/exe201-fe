...
import { getAdminUsers, setUserStatus, approveShop, getAdminProducts, getAdminOrders, getAdminReports } from '../services/admin.js';
import { getPlatformConfig, updatePlatformConfig, getComplaintsList, updateComplaintStatus, getActivityLogs } from '../services/platform.js';
...
const AdminDashboard = () => {
  ...
  const loadData = async () => {
    try {
      const [rep, custs, shps, prods, ords, config, compls, activity] = await Promise.all([
        getAdminReports(),
  ...
  const handleSaveConfig = async (e) => {
    ...
    try {
      ...
      setMessage('Đã lưu cấu hình phí nền tảng & banner.');
      loadData();
    } catch (err) {
  ...
  const handleResolveComplaint = async (e) => {
    ...
    try {
      setMessage('');
      await updateComplaintStatus(resolvingComplaintId, { resolution: resolutionText, status: 'resolved' });
      setMessage('Đã giải quyết khiếu nại thành công.');
      setResolvingComplaintId(null);
      setResolutionText('');
      loadData();
    } catch (err) {
      setError('Không thể giải quyết khiếu nại.');
    }
  };
  ...
  const menuItems = [
    ...
    { id: 'shops', label: 'Cửa hàng (Shops)', icon: '🏬' },
    { id: 'costumes', label: 'Trang phục', icon: '👗' },
    { id: 'orders', label: 'Đơn đặt thuê', icon: '☰' },
    { id: 'categories', label: 'Danh mục', icon: '🏷' },
    { id: 'config', label: 'Cấu hình hệ thống', icon: '⚙' },
    { id: 'complaints_logs', label: 'Khiếu nại & Logs', icon: '🛡' },
    { id: 'chat', label: 'Hỗ trợ khách hàng', icon: '💬' }
  ];
  ...
  return (
    <div className="admin-shell" style={{ minHeight: 'calc(100vh - 120px)' }}>
          ...
          <div className="admin-section-grid" style={{ gridTemplateColumns: '1fr 1fr' }}>
            ...
            <article className="card admin-table-card">
              <div className="section-heading compact-heading">
                ...
                <h2>Khiếu nại giữa khách hàng & Shop</h2>
              </div>
              <div style={{ display: 'grid', gap: '15px', padding: '10px' }}>
                {complaints.map((c) => (
                  <div key={c._id} style={{ border: '1px solid var(--border)', borderRadius: '16px', padding: '16px', background: '#f8fafc' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                      <strong>{c.title}</strong>
                      <span className={`status-pill ${c.status === 'resolved' ? 'active' : 'inactive'}`} style={{ fontSize: '0.7rem' }}>
                        {c.status}
                      </span>
                    </div>
                    <p style={{ color: 'var(--muted)', fontSize: '0.85rem', margin: '6px 0' }}>{c.description}</p>
                    <div style={{ fontSize: '0.78rem', color: 'var(--muted)', marginTop: '6px', borderTop: '1px solid var(--border)', paddingTop: '6px' }}>
                      Người báo: <strong>{c.user?.fullName}</strong> • Bị khiếu nại: <strong>{c.shop?.fullName}</strong>
                    </div>
                    {c.resolution ? (
                      <div style={{ marginTop: '10px', background: '#ecfdf5', padding: '8px', borderRadius: '8px', fontSize: '0.8rem', border: '1px solid #bbf7d0', color: '#166534' }}>
                        <strong>Cách giải quyết:</strong> {c.resolution}
                      </div>
                    ) : (
                      <button 
                        onClick={() => { setResolvingComplaintId(c._id); setResolutionText(''); }} 
                        className="button"
                ...
                {complaints.length === 0 && <div className="empty-state">Chưa nhận được khiếu nại tranh chấp nào.</div>}
              </div>
            </article>
      ...
      {/* RESOLVE COMPLAINT DIALOG OVERLAY */}
      {resolvingComplaintId && (
        <div style={{
          ...
          <form onSubmit={handleResolveComplaint} className="card" style={{
            width: 'min(500px, 100%)', background: 'white', borderRadius: '24px', padding: '30px', position: 'relative'
          }}>
            <button type="button" onClick={() => setResolvingComplaintId(null)} style={{ position: 'absolute', top: '20px', right: '20px', width: '36px', height: '36px', borderRadius: '50%', background: 'var(--surface-soft)', border: '0', fontSize: '1.2rem', display: 'grid', placeItems: 'center' }}>×</button>
            <h2 style={{ margin: '0 0 15px' }}>Giải quyết khiếu nại</h2>
            
            <div className="input-group">
              <label style={{ fontWeight: '800' }}>Biện pháp xử lý / Lời giải quyết</label>
              <textarea 
                placeholder="Ví dụ: Đã hoàn trả tiền đặt cọc 100% cho khách hàng do trang phục rách nát trước nhận..." 
            ...
            <div style={{ display: 'flex', gap: '10px', marginTop: '20px', justifyContent: 'flex-end' }}>
              <button className="primary-button" type="submit">Giải quyết hoàn tất</button>
              <button type="button" onClick={() => setResolvingComplaintId(null)} className="secondary-button">Hủy bỏ</button>
            </div>
          </form>
        </div>
      )}
